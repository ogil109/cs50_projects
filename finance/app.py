...
def index():
    ...
    if request.method == "POST":
        ...
        db.execute("UPDATE users SET cash = cash + ?", topup)
    ...
    return render_template("index.html", stocks=stocks, cash=cash, total_value=total_value, total=total, topup=topup)
...
def buy():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            return apology("Provide stock ticker")
        elif not shares or not shares.isdigit() or int(shares) <= 0:
            return apology("Provide a positive number of shares")
...
def register():
    ...
    # User reached route via POST via the form
    if request.method == "POST":
